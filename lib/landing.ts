...
export function visibleLandingSections(
  gym: Partial<Pick<Gym, "landing_sections" | "benefits" | "gallery" | "address" | "whatsapp" | "testimonials" | "class_schedule">>,
  plans: MemberPlan[]
): LandingSectionKey[] {
  ...
  const hasData: Record<LandingSectionKey, boolean> = {
    ...
    testimonios: (gym.testimonials || []).length > 0,
    horarios: (gym.class_schedule || []).length > 0,
